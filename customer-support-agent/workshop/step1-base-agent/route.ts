...
//   • How to enforce structured output (JSON) via prompt
//   • How to validate the response with Zod
//
// What is NOT here yet (coming in the next steps):
...
const responseSchema = z.object({
  thinking: z.string(),           // agent's internal reasoning (visible in the back office)
  response: z.string(),           // message displayed to the customer
  user_mood: z.enum([
...
// ── 4. JSON Parser ────────────────────────────────────────────────────────────
// Claude sometimes wraps the JSON in markdown (```json ... ```).
